...
namespace Bald::Events {
    ...
    public:
        ...
        static void Subscribe(Function&& callback, Args&& ... args) {
            BALD_STATIC_ASSERT(std::is_base_of_v<Event, EventType>, "EventType must be derived from Event!");
            if constexpr(std::is_member_function_pointer_v<Function>) {
                Unpack<EventType>(std::forward<Function>(callback), std::forward<Args>(args)...);
            } else {
        ...
        template<typename EventType>
        static void Emit(const EventType& e) {
    ...
    private:
        template<typename EventType>
        static void Dispatch(const EventType& e) {
        ...
        template<typename EventType, typename Function, typename Object, typename ... Args>
        static void Unpack(Function&& callback, Object&& object, Args&& ... args) {
            auto callbackWrapper = [callback = std::forward<Function>(callback), object = std::forward<Object>(
                object), args ...](const EventType& e) {
